...
public record ClipboardSyncProperties(
    ...
    public ClipboardSyncProperties {
        devicePublicKeys = devicePublicKeys == null ? Map.of() : Map.copyOf(devicePublicKeys);
        devicePublicKeys.forEach((deviceKey, publicKey) -> {
            if (!StringUtils.hasText(deviceKey) || !StringUtils.hasText(publicKey)) {
