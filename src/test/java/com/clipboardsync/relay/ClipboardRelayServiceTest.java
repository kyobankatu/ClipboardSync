...
class ClipboardRelayServiceTest {
    ...
    void rejectsMessageFromDifferentAuthenticatedDevice() {
        ClipboardMessage message = message("device-a", MessageType.CLIPBOARD_UPDATE);

        assertThatThrownBy(() -> relayService.validateRelayMessage("device-b", message))
    ...
    void rejectsNonUpdateMessage() {
        ClipboardMessage message = message("device-a", MessageType.CLIPBOARD_SNAPSHOT);

        assertThatThrownBy(() -> relayService.validateRelayMessage("device-a", message))
                .isInstanceOf(IllegalArgumentException.class)
    ...
    private static ClipboardMessage message(String sourceDeviceId, MessageType type) {
